...
final class SRKPICS_Quote_Pro {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_srkqp_delete_quote', array($this, 'ajax_delete_quote'));
        ...
        <div id="srkqp-popup" class="srkqp-popup" aria-hidden="true">
            ...
            <div class="srkqp-modal" role="dialog" aria-modal="true" aria-labelledby="srkqp-title">
                ...
                <form id="srkqp-form">
                    ...
                    <input type="text" name="srkqp_company" value="" tabindex="-1" autocomplete="off" style="position:absolute;left:-9999px;top:auto;width:1px;height:1px;overflow:hidden;" aria-hidden="true">
    ...
    public function ajax_submit_quote() {
        /*
         * Guest-safe quote submission.
         * Logged-in users still get nonce validation.
         * Public/non-logged submissions are protected with honeypot + IP rate limit,
         * because full-page cache can serve stale nonces to guests.
         */
        if (is_user_logged_in()) {
            if (
                empty($_POST['nonce']) ||
                !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'srkqp_quote_nonce')
            ) {
        ...
        if (!empty($_POST['srkqp_company'])) {
            wp_send_json_error(array(
                'message' => 'Unable to submit the quote request. Please try again.'
            ), 400);
        }

        $ip_for_rate = sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
        $rate_key = 'srkqp_quote_rate_' . md5($ip_for_rate);
        ...
        if (!$inserted) {
            $this->log_activity('Quote insert failed: ' . $wpdb->last_error, 'error');
                ...
                <table class="widefat fixed striped srkqp-table">
                    ...
                    <?php if ($rows) : foreach ($rows as $row) : ?>
                        <tr>
                            ...
                            <td>
                                <button type="button" class="button srkqp-delete-btn" data-id="<?php echo esc_attr($row->id); ?>">Delete</button>
                            </td>
                        </tr>
                    <?php endforeach; else : ?>
                        <tr><td colspan="8">No quote requests found.</td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
    ...
    public function ajax_delete_quote() {
        ...
        if (!$deleted) {
            wp_send_json_error(array('message' => 'Delete failed. Please try again.'));
        }

        $this->log_activity('Quote #' . $id . ' deleted', 'delete', $id);
